...
function unbalancedDelimiters(lines: Line[]): boolean {
  ...
  const re_enumeration_markers_numeric =
    /(?<!(?:Pkt\.(?: \d+ (?:(?:u\.)|(?:und)))?)|(?:§( \d+,)*)|(?:§ \d+ u\.)|(?:§ \d+, \d+ u\.))( )(\d+\))/g;

  matches.push(...getMatchesCallback(linesMinusOne, (line) => {
    let valueClean = line.value
      .replaceAll(re_enumeration_markers_alphabetic, "$1")
      .replaceAll(re_enumeration_markers_numeric, "$1");
  ...
  matches.push(...getMatches(lines, re_opening_slash_not_followed_by_closing));
  ...
  matches.push(...getMatches(lines, re_closing_slash_not_preceeded_by_opening));
